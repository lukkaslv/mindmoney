        ...
        <div className="flex flex-col items-center justify-center min-h-[70vh] space-y-12 px-4">
          ...
          <div className="w-full space-y-4">
            <input 
              ...
              className="w-full p-6 bg-white border-2 border-slate-50 rounded-[2rem] text-center font-bold text-lg focus:border-indigo-100 outline-none transition-all shadow-inner"
        ...
        <div className="space-y-12 pb-12">
          ...
          {loading ? (
            <div className="game-card p-12 flex flex-col items-center justify-center space-y-10 min-h-[450px]">
              ...
              <p className="text-slate-500 font-bold text-sm text-center leading-relaxed h-16 px-6 animate-pulse">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {audioBase64 && (
                      <button 
                        ...
                        className={`w-14 h-14 rounded-full flex items-center justify-center transition-all ${isPlaying ? 'bg-indigo-50 text-indigo-600' : 'bg-indigo-600 text-white shadow-xl shadow-indigo-100'}`}
                      >
                        {isPlaying ? <div className="flex items-end h-4 gap-0.5"><div className="audio-wave-bar" style={{animationDelay:'0.1s'}}></div><div className="audio-wave-bar" style={{animationDelay:'0.3s'}}></div><div className="audio-wave-bar" style={{animationDelay:'0.5s'}}></div></div> : '▶'}
                      </button>
                    )}
                  </div>
      ...
      <Layout lang={lang}>
        <div className="flex flex-col space-y-6 scene-transition h-full safe-area-bottom">
          <div className="game-card p-8 flex-1 flex flex-col space-y-10">
              ...
              <div className="grid grid-cols-2 gap-3">
                {Object.entries(t.bodySensations).map(([key, label]) => (
                  <button 
                    ...
                    className={`p-5 rounded-[1.8rem] text-[11px] font-black transition-all border-2 ${intermediateFeedback.bodySensation === label ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-50 border-transparent text-slate-500'}`}
                  >
                    {label}
                  </button>
                ))}
            ...
            <div className="space-y-5 flex-1 flex flex-col">
              ...
              <textarea 
                ...
                placeholder="Что вы чувствуете в этот момент?"
                className="w-full flex-1 p-7 bg-slate-50 rounded-[2.5rem] text-sm focus:ring-4 focus:ring-indigo-500/5 outline-none transition-all resize-none font-medium border-none shadow-inner"
              />
            </div>
          </div>
          ...
          <button 
            ...
            className={`w-full py-6 rounded-[2rem] font-black shadow-xl uppercase text-xs tracking-widest transition-all ${intermediateFeedback.bodySensation ? 'bg-indigo-600 text-white shadow-indigo-100' : 'bg-slate-200 text-slate-400 pointer-events-none'}`}
          >
            {t.saveNext}
          </button>
        </div>
    ...
    <Layout lang={lang}>
      <div className={`space-y-10 transition-all duration-500 ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
        <div className="px-2 space-y-3">
           <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-[0.3em] opacity-30">
          ...
          <div className="grid gap-4">
            {scene.choices.map((choice) => (
              <button 
                ...
                className={`w-full p-7 text-left rounded-[2rem] flex items-center transition-all border-2 ${selectedChoiceId === choice.id ? 'bg-indigo-600 border-indigo-600 text-white shadow-xl shadow-indigo-200' : 'bg-white border-slate-50 text-slate-700 active:scale-[0.98] shadow-sm'}`}
              >
                <span className="font-black text-[14px] leading-tight flex-1">{choice.text}</span>
